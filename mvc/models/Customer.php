...
class Customer extends connection
{
    ...
    function registerUser($data)
    {
        ...
        $sqlGetEmail = "SELECT count(uEmail) as totalEmail from `user` WHERE uEmail = " . "'$email'" . "AND uIsDelete = 0";
    ...
    function getOTP()
    {
        ...
        if (filter_var($email, FILTER_VALIDATE_EMAIL)) {
            $sqlGetEmail = "SELECT uID,count(uEmail) as totalEmail from `user` WHERE uEmail = '{$email}' and uStatus = 2 GROUP BY uID";
    ...
    function submitOTP()
    {
        ...
        //uOtpCode,uTimeActiveOtp,uUpdateAt
        $sql = "select * from user where uEmail = '{$email}' and uOtpCode = '{$otp}' LIMIT 1";
        $result = $this->connection->query($sql);
        $otpDetail = $result->fetch_assoc();
        if (isset($otpDetail['uOtpCode'])) {
            ...
            if ($minutes > 5) {
                ...
                $sqlUpdateOtp = "update user set uUpdateAt = '{$timeUpdate}', uStatus = 0 where uID = " . $otpDetail['uID'];
                $this->connection->next_result();
                $result = $this->connection->query($sqlUpdateOtp);
                if ($result) {
                    $clear = new DefaultFunction();
